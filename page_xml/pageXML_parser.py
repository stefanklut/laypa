...
from page_xml.pageXML_creator import convert_to_dict
...
class PageXMLParser:
    ...
    def get_size(self):
        ...
        img_width_str = self.root.findall("".join(["./", self.base, "Page"]))[0].get("imageWidth")
        if img_width_str is None:
            raise ValueError("imageWidth attribute is missing in the XML file.")
        img_width = int(img_width_str)
        img_height_str = self.root.findall("".join(["./", self.base, "Page"]))[0].get("imageHeight")
