...
# ARG_DEFAULTS_POS([])
# ARG_HELP([<Script to run all steps of the baseline/region pipeline, with start and end in a single image>])
# ARGBASH_GO()
...
print_help()
{
	printf '%s\n' "<Script to run all steps of the baseline/region pipeline>"
...
docker run --rm -v $output_dir:$output_dir -v $tmp_dir:$tmp_dir loghi/docker.loghi-tooling:latest /src/loghi-tooling/minions/target/appassembler/bin/MinionExtractBaselinesStartEndNew3 \
